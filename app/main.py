...
from .schemas import (
    ...
    StudentGradingResultResponse, StudentGradingResultDetail
)
...
@app.get("/api/student/{student_id}/results", response_model=List[StudentGradingResultResponse])
async def get_student_results(student_id: str, db: Session = Depends(get_db)):
    """Get all grading results for a specific student"""
    try:
        # Find answer sheets for this student
        answer_sheets = db.query(models.AnswerSheet).filter(
            models.AnswerSheet.student_id == student_id
        ).all()
        ...
        results = []
        for sheet in answer_sheets:
            grading_results = db.query(models.GradingResult).filter(
                models.GradingResult.answer_sheet_id == sheet.id
            ).all()
            ...
            for result in grading_results:
                ...
                if sheet.marking_scheme_id:
                    marking_scheme = db.query(models.MarkingScheme).filter(
                        models.MarkingScheme.id == sheet.marking_scheme_id
                    ).first()
                
                results.append(StudentGradingResultResponse(
                    id=result.id,
...
@app.get("/api/student/{student_id}/results/{result_id}", response_model=StudentGradingResultDetail)
async def get_student_result_detail(student_id: str, result_id: int, db: Session = Depends(get_db)):
    """Get detailed grading result for a specific student and result"""
    try:
        # Verify the result belongs to the student
        result = db.query(models.GradingResult).filter(
            models.GradingResult.id == result_id
        ).first()
        
        if not result:
            raise HTTPException(status_code=404, detail="Result not found")
        
        # Check if the answer sheet belongs to this student
        answer_sheet = db.query(models.AnswerSheet).filter(
            models.AnswerSheet.id == result.answer_sheet_id,
            models.AnswerSheet.student_id == student_id
        ).first()
        
        if not answer_sheet:
            raise HTTPException(status_code=403, detail="Access denied - result does not belong to this student")
        
        # Get marking scheme details through the answer sheet
        marking_scheme = None
        if answer_sheet.marking_scheme_id:
            marking_scheme = db.query(models.MarkingScheme).filter(
                models.MarkingScheme.id == answer_sheet.marking_scheme_id
            ).first()
        
        return StudentGradingResultDetail(
            id=result.id,
            student_id=student_id,
            marking_scheme_id=answer_sheet.marking_scheme_id,
            answer_sheet_id=result.answer_sheet_id,
            total_score=result.total_score,
            total_max_score=result.total_max_marks,
            percentage=result.percentage,
            feedback=result.feedback,
            recommendations=result.recommendations,
            detailed_results=result.question_scores,
            created_at=result.created_at,
            marking_scheme_name=marking_scheme.filename if marking_scheme else "Unknown",
        ...
        logger.error(f"Error getting student result detail: {e}")
        raise HTTPException(status_code=500, detail="Failed to get student result detail")
